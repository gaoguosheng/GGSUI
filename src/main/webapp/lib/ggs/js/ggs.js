...
var $GGS={
	...
	bootstrap:{
		 ...
		 * 创建导航栏
		 * */
		navBar:function(nodes,id){
			var navbar =document.createElement("div");
	    	$(navbar).toggleClass("navbar");
	    	var navbarInner =document.createElement("div");
	    	$(navbarInner).toggleClass("navbar-inner");
	    	var navNode = this.nav(nodes);
	    	$(navbar).append(navbarInner);
	    	$(navbarInner).append(navNode);
	    	$("#"+id).append(navbar);
		},		
		/**
		 * 创建导航菜单
		 * */
	    nav:function(nodes,id,isTab){	    	       	       
	            ...
	            for(var i in nodes){
	                if(nodes[i].pId==pId){
	                    ...
	                    if(hasChild){
	                        ...
	                        $(li).append('<a href="#" class="dropdown-toggle" data-toggle="dropdown">'+p_name+'<b class="caret"></b></a>');
	                    	...
	                    	if(isTab){
	                    		li="<li><a href='#' onclick='$GGS.bootstrap.addTab("+p_id+",\""+p_name+"\",\""+nodes[i].url+"\");'>"+p_name+"</a></li>";	
	                    	}else{
	                    		li="<li><a href='"+nodes[i].url+"' >"+p_name+"</a></li>";
	                    	}
	        ...
	        function createItem(node,container){
	            var hasChild=false;
	            for(var i in nodes){
	                if(nodes[i].pId==node.id){
	                    hasChild=true;
	                    break;
	                }
	            }
	            ...
	            if(hasChild){
	                li=document.createElement("li");
	                $(li).toggleClass("dropdown");
	                $(li).append('<a href="#" class="dropdown-toggle" data-toggle="dropdown">'+node.name+'<b class="caret-right"></b></a>');
	                ul =document.createElement("ul");
	                $(ul).toggleClass("dropdown-menu");
	                $(li).append(ul);
	            }else{
	            	if(isTab){
	            		li="<li><a href='#' onclick='$GGS.bootstrap.addTab("+node.id+",\""+node.name+"\",\""+node.url+"\");'>"+node.name+"</a></li>";	
	            	}else{
	            		li="<li><a href='"+node.url+"'>"+node.name+"</a></li>";
	            	}
	    ...
	    tabs:function(id){
	        ...
	        $(tabDiv).toggleClass("GGS-nav-tabs tabbable");
	        var ul = document.createElement("ul");
	        $(ul).toggleClass("nav nav-tabs");
	        var contentDiv = document.createElement("div");
	        $(contentDiv).toggleClass("tab-content");
	        $(tabDiv).append(ul);
	        $(tabDiv).append(contentDiv);
	        $("#"+id).append(tabDiv);	        
	    },
	    /**
	     * 增加标签页
	     * */
	    addTab:function (id,title,url,hasNotClose) {
	        var tabs = $(".GGS-nav-tabs");	       
	        var tab=$('.GGS-nav-tabs a[href="#'+id+'"]');
	        ...
	        var li="<li><a href='#"+id+"' data-toggle='tab' >"+title;
	        if(!hasNotClose){
	            //不可关闭标签
	            li+="&nbsp;&nbsp;<i title='关闭标签' class='close' onclick='$GGS.bootstrap.closeTab(\""+id+"\");'>&times;</i>";
	        }
	        li+="</a></li>";
	        tabs.find( ".nav-tabs" ).append(li);
	        var mheight=230;
	        var frameHeight=screen.availHeight-mheight;
	        tabs.find(".tab-content").append( "<div class='tab-pane' id='" + id + "'><iframe src='"+url+"' width='100%' height='"+frameHeight+"px' frameborder='0' scrolling='auto'></iframe></div>" );
	        tab=$('.GGS-nav-tabs a[href="#'+id+'"]');
	        tab.tab('show');        
	    ...
	    closeTab:function (id){
	        var tabs = $(".GGS-nav-tabs");
	        var prevTab= tabs.find( ".nav-tabs li > a[href='#"+id+"']").parent().prev();
	        tabs.find( ".nav-tabs li > a[href='#"+id+"']").parent().remove();
	        tabs.find( ".tab-content div[id='"+id+"']").remove();
	    ...
	    dialog:function(options){
	        var dialogstr='<div class="GGS-dialog modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">';
            dialogstr+='<div class="modal-header">';
            dialogstr+='<button type="button" class="close" data-dismiss="modal" aria-hidden="true" title="关闭窗口">×</button>';
            dialogstr+='<h3></h3>';
            dialogstr+='</div>';
	        dialogstr+='<div class="modal-body"></div>';
	        dialogstr+='<div class="modal-footer"></div>';
	        dialogstr+='</div>';
	        
	        var mydialog = $("#"+options.id);
	        if(mydialog.length==0){       		        	
	        	mydialog=document.createElement("div");
	        	$(mydialog).attr("id",options.id);	        	
	        	$(document.body).append(mydialog);
	        }
	        $(mydialog).html(dialogstr);	
	        
	        var width=600,height=300,top=20;
	        ...
	        $(mydialog).find(".GGS-dialog").css("width",width);
	        $(mydialog).find(".GGS-dialog").css("left",((screen.availWidth-width)/2)+"px");
	        $(mydialog).find(".GGS-dialog").css("margin-left","0px");
	        $(mydialog).find(".GGS-dialog >.modal-body").css("height",height+"px");
	        $(mydialog).find(".GGS-dialog").css("top",top+"px");
	        $(mydialog).find(".GGS-dialog").css("margin-top","0px");
	        $(mydialog).find(".GGS-dialog >.modal-header >h3").html(options.title);	  
	        $(mydialog).find(".GGS-dialog >.modal-header >h3").css("font-size","12pt");	 
	        if(options.content){
	        	$(mydialog).find(".GGS-dialog >.modal-body").html(options.content);	
	        }else if(options.url){
	        	var html =$GGS.ajax.sync(options.url,options.data);
	        	$(mydialog).find(".GGS-dialog >.modal-body").html(html);
	        }
	        
	        
	        var foot = $(mydialog).find(".GGS-dialog >.modal-footer");
	        $(foot).html("");
	        var btns = options.buttons;
	        if(btns){
	        	 for(var i in btns){
	 	            var newbtn =document.createElement("button");
	 	            $(newbtn).toggleClass(btns[i].className);
	 	            $(newbtn).html(btns[i].name);
	 	            $(newbtn).click(btns[i].click);
	 	            $(foot).append(newbtn);
	 	        }	        	
	        }     	       	        
	        var backdrop=false;
	        ...
	        $(mydialog).find('.GGS-dialog').modal({
	            backdrop:backdrop,
	            keyboard:true,
	            show:true
	        });	        
	    ...
	    * */
	    alert:function (msg){	    	
	    	this.dialog({
	    		...
	    		height:60,
	    		buttons:[
                              {name:"关闭",className:"btn",click:function(){
                            	  $GGS.bootstrap.closeDialog("ggs-alert");
                              }}
                             ]
	    	});
	    },
	    ...
	    confirm:function(msg,fun){	    	
	    	this.dialog({
	    		...
	    		height:60,
	    		buttons:[
								{name:"确定",className:"btn btn-primary",click:function(){
									  fun();
									  $GGS.bootstrap.closeDialog("ggs-confirm");
								}},
	                            {name:"取消",className:"btn",click:function(){
	                            	  $GGS.bootstrap.closeDialog("ggs-confirm");
	                            }}
                             ]
	    	});
    ...
    tips:function(msg,times){
    	$._messengerDefaults = {
    			extraClasses: 'messenger-fixed messenger-theme-ice  messenger-on-bottom messenger-on-right'
    		};
    	if(!times)
    		times=3;
